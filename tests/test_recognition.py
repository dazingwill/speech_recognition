...
class TestRecognition(unittest.TestCase):
    ...
    @unittest.skipUnless("IBM_KEY" in os.environ, "requires IBM Speech to Text username and password to be specified in IBM_USERNAME and IBM_PASSWORD environment variables")
    def test_ibm_english(self):
        r = sr.Recognizer()
        with sr.AudioFile(self.AUDIO_FILE_EN) as source: audio = r.record(source)
        region = os.environ["IBM_REGION"] if "IBM_REGION" in os.environ else "stream"
        self.assertEqual(r.recognize_ibm(audio, key=os.environ["IBM_KEY"], region=region), "one two three ")
    ...
    def test_ibm_french(self):
        ...
        with sr.AudioFile(self.AUDIO_FILE_FR) as source: audio = r.record(source)
        region = os.environ["IBM_REGION"] if "IBM_REGION" in os.environ else "stream"
        self.assertEqual(r.recognize_ibm(audio, key=os.environ["IBM_KEY"], region=region, language="fr-FR"), u"si la dictée numéro un ")
    ...
    def test_ibm_chinese(self):
        ...
        with sr.AudioFile(self.AUDIO_FILE_ZH) as source: audio = r.record(source)
        region = os.environ["IBM_REGION"] if "IBM_REGION" in os.environ else "stream"
        self.assertEqual(r.recognize_ibm(audio, key=os.environ["IBM_KEY"], region=region, language="zh-CN"), u"砸 自己 的 脚 ")

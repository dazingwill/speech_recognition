...
class Recognizer(AudioSource):
    ...
    def recognize_ibm(self, audio_data, key, region="stream", language="en-US", show_all=False):
        """
        The IBM Speech to Text API key are specified by ``key``. Unfortunately, these are not available without `signing up for an account <https://console.bluemix.net/registration/>`__. Once logged into the Bluemix console, follow the instructions for `creating an IBM Watson service instance <https://cloud.ibm.com/docs/services/watson?topic=watson-creating-credentials>`__, where the Watson service is "Speech To Text". IBM Speech to Text API keys are mixed-case alphanumeric strings.

        The recognition language is determined by ``language``, an RFC5646 language tag with a dialect like ``"en-US"`` (US English) or ``"zh-CN"`` (Mandarin Chinese), defaulting to US English. The supported language values are listed under the ``model`` parameter of the `audio recognition API documentation <https://cloud.ibm.com/apidocs/speech-to-text>`__, in the form ``LANGUAGE_BroadbandModel``, where ``LANGUAGE`` is the language value.

        Returns the most likely transcription if ``show_all`` is false (the default). Otherwise, returns the `raw API response <https://cloud.ibm.com/docs/services/speech-to-text?topic=speech-to-text-http>`__ as a JSON dictionary.
        ...
        url = "https://{}.watsonplatform.net/speech-to-text/api/v1/recognize?{}".format(region, urlencode({
        ...
        authorization_value = base64.standard_b64encode("{}:{}".format("apikey", key).encode("utf-8")).decode("utf-8")
